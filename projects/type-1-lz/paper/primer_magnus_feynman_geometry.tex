...
% selection for Type-1 N=3 multistate Landau-Zener. Companion to paper/ws_geom_scope.md and the
% markdown primer paper/primer_magnus_feynman_geometry.md. Expository; no new claims.
...
energy gaps are large --- the well-separated/generic regime. \emph{Deep overlap} is precisely
$\int\norm{\Wt}\to$ large: the series must be resummed, and that resummation is the hard $\sigma$.
...
The companion scope (\texttt{paper/ws\_geom\_scope.md}) organizes the work into four milestones; here is
...
explicitly, and validate \emph{term by term} against the gold oracle in the generic regime. \emph{Gate:}
the error $|P^{(n)}-P^{\mathrm{oracle}}|$ must (a) decrease with order $n$ at fixed action $\delta$, and (b)
decrease with $\delta$ at fixed order; the deep-overlap non-convergence ($\delta\gtrsim0.5$) is confirmed as
the honest boundary. \emph{Output:} \texttt{experiments/ws\_geom\_magnus.py},
\texttt{paper/ws\_geom\_m1.md}.
